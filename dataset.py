...
# 该文件主要用于数据加载和数据转换，
# 将预处理好的文本音素、时序时间、mel谱图、pitch序列和energy序列等数据转换、加载为模型可以直接使用的形式。
...
if __name__ == "__main__":
    ...
    preprocess_config = yaml.load(
        open("./config/LJSpeech/preprocess.yaml", "r", encoding='utf-8'), Loader=yaml.FullLoader
    )
    train_config = yaml.load(
        open("./config/LJSpeech/train.yaml", "r", encoding='utf-8'), Loader=yaml.FullLoader
    )
